...
package object ex01 extends Exercise01 {
  // Problem 1
  def freeIds(expr: WAE): Set[String] = expr match {
    case Num(num) => Set()
    case Add(left, right) => freeIds(left) | freeIds(right)
    case Sub(left, right) => freeIds(left) | freeIds(right)
    case With(name, expr, body) => freeIds(expr) | (freeIds(body) - name)
    case Id(id) => Set(id)
  }
  // Problem 2
  ...
  def boundIds(expr: WAE): Set[String] = {
    def bounds(expr: WAE, ids: Set[String]): Set[String] = expr match{
      case Num(num) => Set()
